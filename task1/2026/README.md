...
```sh
make up         # собрать образы и поднять весь стек (postgres на 127.0.0.1:55432 + app)
make app        # запустить клиент ВНУТРИ контейнера (интерактивно, в цикле)
make run        # запустить клиент на ХОСТЕ (против 127.0.0.1:55432)
make logs       # лог сервера (виден метод аутентификации)
make reset      # удалить том и заново выполнить db/initdb/*.sql
make nuke       # полный сброс: контейнеры, том, локальные образы, «сироты»
make down       # остановить и удалить стек (том сохраняется)
```
...
он читается напрямую из `/dev/tty`, поэтому `make app` использует
`docker-compose exec` с TTY.

Клиент работает **в цикле**: после каждого отчёта снова спрашивает режим, логин
и пароль, так что подряд можно перебрать много полезных нагрузок. Выход — `Ctrl-D`
(конец ввода).
